...
export default class Shop {
  ...
  updateQuality() {
    this.items = this.items.map(item => {
        ...
        if (shouldDegradeQuality && !hasPassedSellInDay(item.sellIn)) {
          item.quality = item.quality - 1;
        } else if (shouldDegradeQuality && hasPassedSellInDay(item.sellIn)) {
          ...
          if (hasPassedSellInDay(item.sellIn) && !isMaxQuality(item.quality)) {
            item.quality = item.quality + 1;
          }
        }
